...
class AuthVerticle : CoroutineVerticle() {
  override suspend fun start() {
    ...
    //对/api开头的所有接口进行用户认证
    router.route("/api/*")
      .handler(JWTAuthHandler.create(JWTUtil.getJWTAuth()))

    val requireAdminHandler = AuthorizationHandler.create(PermissionBasedAuthorization.create(RoleEnum.ADMIN.roleName))
      .addAuthorizationProvider(JWTAuthorization.create(JWTUtil.PERMISSIONS))
